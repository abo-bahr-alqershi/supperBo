...
export const useHomeScreenBuilder = (options: UseHomeScreenBuilderOptions = {}) => {
  ...
  // Fetch template data
  const { data: template, isLoading: templateLoading, error: templateError } = useQuery<HomeScreenTemplate, Error, HomeScreenTemplate>({
    queryKey: ['homeScreenTemplate', templateId],
    queryFn: () => homeScreenService.getTemplateById(templateId!),
    enabled: !!templateId
  });
