...
namespace YemenBooking.Infrastructure.Services
{
    ...
    public class ExportService : IExportService
    {
        ...
        /// <inheritdoc />
        public async Task<ExportResult> ExportBookingsToExcelAsync(IEnumerable<Booking> bookings, string fileName, ExportOptions? options = null, CancellationToken cancellationToken = default)
        {
            ...
            try
            {
                ...
                var headers = new[] { "Id", "UserId", "UnitId", "CheckIn", "CheckOut", "GuestsCount", "TotalPrice", "Status", "BookedAt", "BookingSource", "CancellationReason", "IsWalkIn", "PlatformCommission" };
                for (int i = 0; i < headers.Length; i++) sheet.Cell(1, i + 1).Value = headers[i];
                for (int idx = 0; idx < data.Count; idx++)
                {
                    var b = data[idx];
                    ...
                    sheet.Cell(row, 4).Value = b.CheckIn;
                    sheet.Cell(row, 5).Value = b.CheckOut;
                    sheet.Cell(row, 6).Value = b.GuestsCount;
                    sheet.Cell(row, 7).Value = b.TotalPrice.ToString();
                    sheet.Cell(row, 8).Value = b.Status.ToString();
                    sheet.Cell(row, 9).Value = b.BookedAt;
                    sheet.Cell(row, 10).Value = b.BookingSource;
                    sheet.Cell(row, 11).Value = b.CancellationReason;
                    sheet.Cell(row, 12).Value = b.IsWalkIn;
                    sheet.Cell(row, 13).Value = b.PlatformCommissionAmount;
                }
                var path = Path.Combine(_exportFolder, fileName);
                workbook.SaveAs(path);
                var info = new FileInfo(path);
