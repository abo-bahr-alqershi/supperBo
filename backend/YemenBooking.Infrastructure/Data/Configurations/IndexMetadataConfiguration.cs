...
namespace YemenBooking.Infrastructure.Data.Configurations
{
    ...
    public class IndexMetadataConfiguration : IEntityTypeConfiguration<IndexMetadata>
    {
        public void Configure(EntityTypeBuilder<IndexMetadata> builder)
        {
            ...
            // قيود التحقق
            builder.HasCheckConstraint("CK_IndexMetadata_Status", 
                "[Status] IN ('Active', 'Building', 'Error', 'Disabled', 'Optimizing')");

            builder.HasCheckConstraint("CK_IndexMetadata_TotalRecords", 
                "[TotalRecords] >= 0");

            builder.HasCheckConstraint("CK_IndexMetadata_FileSizeBytes", 
                "[FileSizeBytes] >= 0");

            builder.HasCheckConstraint("CK_IndexMetadata_Version", 
                "[Version] > 0");

            builder.HasCheckConstraint("CK_IndexMetadata_RebuildAttempts", 
                "[RebuildAttempts] >= 0");
        }
    }
}